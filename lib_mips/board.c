...
#if defined MT7620_FPGA_BOARD  || MT7621_FPGA_BOARD 
...
void OperationSelect(void)
{
	...
	printf( "   %d: System Load UBoot to SDRAM via TFTP. \n", SEL_LOAD_BOOT_SDRAM );
...
/*	int io_scan;
	for ( io_scan = 0; io_scan < 72; io_scan++ ) {
		if ( !mtk7620_get_gpio_pin( io_scan ) )
			printf( "io %d is 0!\n", io_scan );
		else
			printf( "io %d is 1!\n", io_scan );
	}
...
/*
	...
	for ( led_nr = 0; led_nr <= 72; led_nr++ ) {

		printf( "LED nr: %d on   ", led_nr );
		mtk7620_set_gpio_dir( led_nr, 0);
		mtk7620_set_gpio_pin( led_nr, 0 );
		udelay( 2000000 );

		printf( "LED nr: %d off\n", led_nr );
		mtk7620_set_gpio_dir( led_nr, 0);
		mtk7620_set_gpio_pin( led_nr, 1 );
		udelay( 1000000 );
	}
*/
